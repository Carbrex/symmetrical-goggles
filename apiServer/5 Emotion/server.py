...
async def model(sid):
    ...
    while is_silent(data) == False:
        ...
        print('predictions:', pred_np)

        max_emo = np.argmax(predictions)
        print('max emotion:', emotions.get(max_emo, -1))
        sio.emit('emotion', {
            'emotion': '${emotions.get(max_emo, -1)}'}, room=sid)
    ...
    print('total predictions:', total_predictions_np)
    sio.emit('emotion', {
        'emotion': '${total_predictions_np}'}, room=sid)
    ...
    print('model done from model')


@sio.event
def connect(sid, environ):
    print(f"Client {sid} connected")
    sio.event('my_event', sid)
    asyncio.run(model(sid))
    print('model done')
    # model(sid)
